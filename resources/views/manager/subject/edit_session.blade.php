        ...
        <div class="col-12">
            <div class="card">
                ...
                <form id="form_create_post" method="post"
                      action="{{route('admin.session.update')}}"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="card-body row">
                        <input type="hidden" name="id" value="{{$id}}">
                        <div class="col-8">
                            <label for="title">Tên buổi học</label>
                            <input type="text" class="form-control" value="{{$data->name}}" id="title" name="name" placeholder="Tiêu đề">
                        </div>
                        ...
                        <div class="form-group col-2">
                            <img style="border: 1px solid #ccc;margin-right:10px" src="{{asset('images/upload/'.$data->image)}}" id="image-show" width="100" height="100" >
                            <span id="btn-cancel-image" class="btn btn-sm btn-danger" for="image-show">Hủy</span>
                        </div>
                        <div class="col-4">
                            <label for="title">Link 1</label>
                            <input type="text" class="form-control" value="{{$data->link1}}" id="title" name="link1" placeholder="link tài liệu nếu có">
                        </div>
                        <div class="col-4">
                            <label for="title">Link 2</label>
                            <input type="text" class="form-control" value="{{$data->link2}}" id="title" name="link2" placeholder="link tài liệu nếu có">
                        </div>
                        <div class="col-4">
                            <label for="title">Link 3</label>
                            <input type="text" class="form-control" value="{{$data->link3}}" id="title" name="link3" placeholder="link tài liệu nếu có">
                        </div>
                        <div class="col-12 mt-4">
                            <label for="summernote">Nội dung</label>
                            <textarea id="summernote" name="detail">
                                {{json_decode($data->detail,true)}}
                            </textarea>
                        </div>
                    </div>
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#button-store-post').click(function (e) {
                    e.preventDefault();
                    $.ajax({
                        url: '{{route('admin.session.update')}}',
                        type: 'POST',
                        ...
                        success: function (data) {
                            ...
                            setTimeout(function () {
                                window.location.href = '{{route('admin.subject.session',['id'=>$data->chapter_id])}}';
                            }, 3000);
                        },
                        error: function (data) {
                            $.toast({
